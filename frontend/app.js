...
// ===== CTF LOADER v5 (shape-agnostic + offline fallback) =====
(function () {
  const LOCAL = [
  ...
  function normQ(d) {
    ...
    if (Array.isArray(d)) return d[0] ? normQ(d[0]) : null;
    const q = d.question || d.prompt || d.text || d.title || d.q;
    const opts = d.options || d.choices || d.answers;
    if (q && opts) return { question: q, options: opts, answer: (d.answer !== undefined ? d.answer : (d.correct !== undefined ? d.correct : d.answer_index)), explanation: d.explanation || d.explain || "" };
    if (d.bite) return normQ(d.bite);
    if (d.data) return normQ(d.data);
    if (d.questions) return normQ(d.questions);
    for (const k in d) { if (d[k] && typeof d[k] === "object") { const r = normQ(d[k]); if (r) return r; } }
    return null;
  }
  function render(l, q, src) {
    ...
    q.options.forEach(function (opt, i) {
      html += '<label style="display:block;margin:6px 0;cursor:pointer"><input type="radio" name="ctf-opt5" value="' + i + '" style="margin-right:8px">' + escapeHtml(opt) + "</label>";
    });
    html += '<button id="ctf-check5" class="btn-primary" style="margin-top:10px">Check answer</button><p id="ctf-fb5" style="margin-top:10px;display:none"></p></div>';
    l.innerHTML = html;
    l.querySelector("#ctf-check5").addEventListener("click", function () {
      const sel = l.querySelector('input[name="ctf-opt5"]:checked');
      const fb = l.querySelector("#ctf-fb5"); fb.style.display = "block";
      if (!sel) { fb.style.color = "#f59e0b"; fb.textContent = "Pick an option first."; return; }
      const val = parseInt(sel.value, 10);
  ...
  async function tick() {
    ...
    if (!l) { l = document.createElement("div"); l.id = "ctf-list"; l.style.cssText = "max-width:720px;margin-top:12px"; sv.appendChild(l); }
    tries++;
    try {
      const tok = localStorage.getItem("cv_token") || localStorage.getItem("token") || "";
      if (tok) {
        const r = await fetch("/api/ctf/today", { headers: { Authorization: "Bearer " + tok } });
        if (r.ok) {
          const q = normQ(await r.json());
          if (q) { done = true; render(l, q, "live"); return; }
        }
      }
    } catch (e) {}
    if (tries >= 3 && !done) {
      done = true;
      render(l, LOCAL[new Date().getDate() % LOCAL.length], "offline-bank");
    }
  }
