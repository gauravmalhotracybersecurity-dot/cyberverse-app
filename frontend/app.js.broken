...
(function () {
  ...
  function authFetch(ep) {
    const q = normQ(await api("/api/ctf/today"));
...
(function () {
  ...
  if (bd) bd.onclick = close;
  document.querySelectorAll(".nav-item").forEach(function (b) { b.addEventListener("click", function () { if (window.innerWidth <= 900) close(); }); });
})();
...
(function () {
  ...
  if (localStorage.getItem("cv_token") && localStorage.getItem("cv_seen_news") !== V) {
    ...
    setTimeout(function () { if (window.toast) toast("New this week: Lab Log, Story Bank & Roadmap", "success"); }, 1800);
  }
})();
// ===== API SHIM (JWT autodiscovery) =====
...
(function () {
  let loaded = false;
  function sec() { return document.getElementById("view-league") || document.getElementById("view-leaderboard"); }
  function listEl() {
    let l = document.getElementById("league-list") || document.getElementById("leaderboard-list");
    const sv = sec();
    if (!l && sv) { l = document.createElement("div"); l.id = "league-list"; l.style.cssText = "max-width:720px;margin-top:12px"; sv.appendChild(l); }
  ...
  async function tryLoad() {
    const sv = sec(); if (!sv || loaded) return; loaded = true;
    const l = listEl(); if (!l) return;
    l.innerHTML = "Loading weekly league...";
    try {
      const data = await api("/api/leaderboard");
      const users = (data && (data.users || data.leaders)) || (Array.isArray(data) ? data : []);
      if (!users.length) { l.innerHTML = "<p>No league members yet.</p>"; return; }
      const medals = ["🥇", "", ""];
      let html = '<table style="width:100%;border-collapse:collapse"><thead><tr><th style="padding:8px;text-align:left">#</th><th style="padding:8px;text-align:left">Player</th><th style="padding:8px;text-align:right">XP</th></tr></thead><tbody>';
      users.forEach(function (u, i) {
        html += '<tr style="border-bottom:1px solid #222"><td style="padding:10px 8px">' + (medals[i] || (i + 1)) + '</td><td style="padding:10px 8px">' + escapeHtml(u.name || "Anonymous") + '</td><td style="text-align:right;padding:10px 8px;color:var(--accent)">' + (u.xp || 0) + '</td></tr>';
      });
...
(function () {
  ...
  function getBox() {
    ...
    if (l) return l;
    const sv = document.getElementById("view-ctf");
    if (!sv) return null;
