...
def billing_verify(payload: dict, user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.commit()
    return {"ok": True, "plan": plan, "method": "signature" if ok_sig else "api_fallback"}
...
if i0 > -1 and i1 > i0:
    c = c[:i0] + new_verify + c[i1:]
    open(ar, "w", encoding="utf-8").write(c)
    print("[BACKEND] verify v2 with API fallback installed")
else:
    print("[ERROR] anchors not found")

subprocess.run(["git", "add", "-A"])
subprocess.run(["git", "commit", "-m", "Payments: verify v2 with Razorpay API fallback (no paid user left locked)"])
